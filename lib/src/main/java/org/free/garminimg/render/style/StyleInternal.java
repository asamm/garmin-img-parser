...
import org.free.garminimg.render.paint.*;
import org.free.garminimg.render.paint.PaintPattern.PatternStyle;
import org.free.garminimg.render.painters.*;
...
import static org.free.garminimg.utils.ImgConstants.*;
...
public class StyleInternal {
    ...
    private PointDrawSpec createPointSpec(int type) {
        ...
        switch (type) {
            ...
            case 0x2f0f:
                ...
                break;
            case 0x2f10:
                result = finishPointSpec(0x2f10, "Service, Personal");
                result = finishPointSpec(0x2f11, "Service, Business");
                break;
            case 0x2f12:
                result = finishPointSpec(0x2f12, "Service, Communication");
                break;
            case 0x2f13:
                result = finishPointSpec(0x2f13, "Service, Repair");
                result = finishPointSpec(0x2f14, "Service, Social");
                break;
            case 0x2f15:
    ...
    private PointDrawSpec finishPointSpec(int type, String name, String iconName, int priority) {
        ...
        spec.setDrawLabelsParams(12, ZOOM_MAX);
        return spec;
    }
    ...
    private LinePolyDrawSpec createPolySpec(int type) {
        LinePolyDrawSpec result;
        switch (type) {
            case 0x06: // Residential street
//                result = finishPolySpec(type, 10, "Parking garage",
//                        Color.GRAY);
                result = finishLineSpec(type, 5, "Street",
                        1.5f, Color.parseColor("#FFFFFF"));
                break;
            case AIRPORT:
...
//// Hlavní tahy (často chybí a padají do fialové)
//            case 0x01: // Motorway
//            case 0x0a: // Trunk
//                result = finishLineSpec(type, 10, "Major Road", 3.0f, Color.parseColor("#E9A6AD"));
//                break;
//            case 0x16: // Walkway / Path
//                result = finishLineSpec(type, 1, "Path", 0.8f, Color.parseColor("#999999"));
//                //((LinePainterSimple) result.getPainter()).setDash(new float[]{5, 5}); // Přerušovaná čára
//                break;
            default:
                // Fallback logika pro plochy
                int baseType = type & 0xFF; // Získáme základní typ bez subtypu

                if (baseType >= 0x01 && baseType <= 0x05) { // Obytné/městské zóny
                    result = finishPolySpec(type, POLY_PRIOR_BG_AREAS, "Urban Area", Color.parseColor("#EEE9E3"));
                } else if (baseType >= 0x14 && baseType <= 0x16) { // Přírodní oblasti / Parky
                    result = finishPolySpec(type, POLY_PRIOR_BG_AREAS + 10, "Green Area", Color.parseColor("#C8FACC"));
                } else if (baseType >= 0x32 && baseType <= 0x53) { // Moře a vodní plochy
                    result = finishPolySpec(type, POLY_PRIOR_BG_AREAS - 10, "Water", Color.parseColor("#AAD3DF"));
                } else {
                    // Skutečně neznámý prvek - neutrální šedá, která neruší
                    result = finishPolySpec(type, Integer.MAX_VALUE, "Unknown area (" + Integer.toHexString(type) + ")",
